...
{
    public class BulkSmsService: ISMSSender
    {       
        ...
        {
            username = "gilbertngandu";
            password = "palacegroup";
            url = "http://bulksms.2way.co.za/eapi/submission/send_sms/2/2.0";
            num_retries = 5;
        }
        public bool SendSMS(List<string> cellnumbers, string msg)
        {
            List<bool> SMSsent = new List<bool>(cellnumbers.Count);
            int index = 0;
            foreach (string cell in cellnumbers)
            {
                string data = seven_bit_message(username, password, cell, msg);
            ...
            return SMSsent.Where(v => v == true).Count() == cellnumbers.Count();
        }
